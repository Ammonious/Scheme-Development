...
extension ColorExtension on Color {
	...
	String get hexString =>
			this == null ? Colors.blue.value.toRadixString(16).substring(2, this.value
					.toRadixString(16)
					.length) : this.value.toRadixString(16).substring(2, this.value
					.toRadixString(16)
					.length);
...
class SmartColor {
	...
	static Color _makeAntDColor(Color color, int index) {
		...
		return HSVColor.fromAHSV(
			1, _getHue(hsv, i, isLight), _getSaturation(hsv, i, isLight), _getValue(hsv, i, isLight),)
				.toColor();
	}
...
class DesignColor extends ColorSwatch<int> {
	const DesignColor(int primary, Map<int, Color> swatch) : super(primary, swatch);
...
class TextColor extends Color {
	static int _getBrandColorFromString(Color backgroundColor) {
		...
		String shadedHex = Color.fromARGB(255, d, d, d).hexString.toUpperCase().replaceAll("#", "");
...
double _getValue(HSVColor hsv, int i, bool isLight) {
	...
	if (isLight == true) {
		return (((hsv.value * 100).round() + brightnessStep1 * i) > 100 ? 100 :
		((hsv.value * 100).round() + brightnessStep1 * i)).toDouble() / 100;
	}
...
colorIsDarker(Color colorOne, Color colorTwo) {
	TinyColor(colorOne);
	;
	return TinyColor(colorOne).getBrightness() < TinyColor(colorTwo).getBrightness();
}
...
MaterialAccentColor generateAccent(Color src) => MaterialAccentColor(
		src.value, _generateAccentSwatch(src));
