...
import 'package:scheme_utilities/src/image_utils/image_cache_manager.dart';
...
class SchemeFileUtils extends GetxController {
  ...
  Future<void> downloadFile(
    ...
    String path = 'No Data';
    Dio dio = Dio();
    PermissionStatus status = await SchemePermission.to.requestStoragePermission();
    if (status == PermissionStatus.granted) {
      ...
      if(onError != null) onError(progress);
    }
  }

  Future<File> saveImageInternal(String url, String imageType) async {
    ...
    return File('$pathName.png')..writeAsStringSync('$pathName.png');
  }

 Future<void> saveToPrefs(imageType, pathName) async {
    final box = GetStorage();
   return box.write(imageType, '$pathName.png');
  }
  ...
  Future<File> getFile({String fileName}) async {
    ...
    return File(box.read<String>(fileName));
  }
  ...
  Future<void> shareFile(File file, {String sheetTitle, String message}) async =>
      ShareExtend.share(file.path, "image");

  Future<dynamic> copyClipboard(String text) => Clipboard.setData(ClipboardData(text: text));
  ...
  static Future<List<FileSystemEntity>> getRecentFiles({bool showHidden}) async {
    ...
    files
        .sort((a, b) => File(a.path).lastAccessedSync().compareTo(File(b.path).lastAccessedSync()));
    ...
    switch (sort) {
      case 0:
        if (list.toString().contains("Directory")) {
          list
            ..sort((f1, f2) =>
                basename(f1.path).toLowerCase().compareTo(basename(f2.path).toLowerCase()));
          return list
            ..sort((f1, f2) => f1
                .toString()
                .split(":")[0]
      ...
      case 1:
        list.sort(
            (f1, f2) => basename(f1.path).toLowerCase().compareTo(basename(f2.path).toLowerCase()));
        if (list.toString().contains("Directory")) {
          list
            ..sort((f1, f2) => f1
                .toString()
      ...
      case 2:
        return list
          ..sort((f1, f2) =>
              FileSystemEntity.isFileSync(f1.path) && FileSystemEntity.isFileSync(f2.path)
